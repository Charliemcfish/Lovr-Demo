  ...
  <div className={`absolute inset-0 z-[200] bg-gradient-to-b from-white via-pink-50 to-pink-100 flex flex-col items-center justify-center animate-out fade-out duration-700 delay-[2800ms] fill-mode-forwards pointer-events-none overflow-hidden`}>
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {[...Array(12)].map((_, i) => (
        <div
          ...
          style={{
            left: `${8 + (i * 7.5)}%`,
            animation: `floatUpSplash ${2.5 + (i % 3) * 0.5}s ease-out infinite`,
    ...
    <style>{`
      @keyframes floatUpSplash {
        ...
        10% {
          opacity: 0.7;
          transform: translateY(-80px) rotate(5deg) scale(1);
        }
        90% {
          opacity: 0.5;
        }
        100% {
          transform: translateY(-800px) rotate(15deg) scale(0.9);
          opacity: 0;
        }
      }
    `}</style>
...
const WelcomeScreen = ({ userName, setUserName, userPreference, setUserPreference, onComplete }) => {
    ...
    return (
        <div className="absolute inset-0 z-[150] bg-gradient-to-br from-pink-100 via-white to-rose-100 flex items-center justify-center p-6 animate-in fade-in duration-500 overflow-hidden">
            {/* Floating hearts animation on continue */}
            {showHearts && (
                <div className="absolute inset-0 pointer-events-none z-50 overflow-hidden">
                    {[...Array(15)].map((_, i) => (
                        <div
                            ...
                            style={{
                                left: `${10 + (i * 5.5)}%`,
                                bottom: '40%',
                                animation: `floatUpOnboard ${1.5 + (i % 3) * 0.3}s ease-out forwards`,
                    ...
                    <style>{`
                        @keyframes floatUpOnboard {
                            ...
                            100% {
                                transform: translateY(-500px) rotate(20deg) scale(0.8);
                                opacity: 0;
                            }
                        }
                    `}</style>
            ...
            <div className="absolute top-10 left-10 w-32 h-32 bg-pink-200 rounded-full blur-[60px] opacity-60 animate-pulse"></div>
            <div className="absolute bottom-20 right-10 w-40 h-40 bg-rose-200 rounded-full blur-[80px] opacity-50 animate-pulse" style={{ animationDelay: '1s' }}></div>
            <div className="absolute top-1/2 left-1/4 w-24 h-24 bg-purple-200 rounded-full blur-[50px] opacity-40 animate-pulse" style={{ animationDelay: '0.5s' }}></div>

            <div className={`bg-white/80 backdrop-blur-xl w-full max-w-sm rounded-[40px] p-8 shadow-2xl flex flex-col items-center text-center border border-white/50 transition-all duration-300 ${isAnimating ? 'scale-95 opacity-50' : 'scale-100 opacity-100'}`}>
                {/* Progress indicator */}
                <div className="flex gap-2 mb-6">
                    <div className={`h-1.5 w-12 rounded-full transition-all duration-500 ${step >= 1 ? 'bg-pink-500' : 'bg-gray-200'}`}></div>
                    <div className={`h-1.5 w-12 rounded-full transition-all duration-500 ${step >= 2 ? 'bg-pink-500' : 'bg-gray-200'}`}></div>
                </div>

                <LovrLogo className="w-32 mb-4" color="brand" />

                {step === 1 && (
                    <div className="w-full animate-in slide-in-from-right duration-300">
                        <div className="mb-6">
                            <h1 className="text-2xl font-black text-gray-900 mb-2">What's your name?</h1>
                            <p className="text-gray-500 text-sm font-medium">Let's get to know you better</p>
                        </div>
                        ...
                        <div className="relative mb-6">
                            <input
                                ...
                                className="w-full bg-gray-50/80 p-5 rounded-2xl font-bold text-gray-900 text-center text-xl border-2 border-transparent focus:border-pink-500 focus:bg-white focus:outline-none transition-all placeholder:text-gray-300 placeholder:font-medium"
                                autoFocus
                                onKeyDown={(e) => e.key === 'Enter' && handleNextStep()}
                            />
                            {userName && (
                                <div className="absolute right-4 top-1/2 -translate-y-1/2">
                                    <Check size={24} className="text-green-500 animate-in zoom-in duration-200" />
                                </div>
                            )}
                        </div>
                        ...
                        <button
                            ...
                            className="w-full py-4 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-2xl font-bold text-lg shadow-lg shadow-pink-200 hover:scale-[1.02] hover:shadow-xl disabled:opacity-40 disabled:scale-100 disabled:shadow-none transition-all duration-300 flex items-center justify-center gap-2"
                        >
                            Continue
                ...
                {step === 2 && (
                    <div className="w-full animate-in slide-in-from-right duration-300">
                        <div className="mb-6">
                            <h1 className="text-2xl font-black text-gray-900 mb-2">Hi {userName}!</h1>
                            <p className="text-gray-500 text-sm font-medium">Who are you interested in meeting?</p>
                        </div>

                        <div className="flex flex-col gap-3 mb-6">
                            <button
                                onClick={() => handlePreferenceSelect('male')}
                                className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${userPreference === 'male' ? 'border-pink-500 bg-pink-50 shadow-lg shadow-pink-100' : 'border-gray-100 bg-gray-50/80 hover:border-gray-200'}`}
                            >
                                <div className={`p-3 rounded-full transition-all ${userPreference === 'male' ? 'bg-pink-500 text-white' : 'bg-white text-pink-500'}`}>
                                    <Mars size={24} />
                                </div>
                                <span className="text-lg font-bold text-gray-800 flex-1 text-left">Men</span>
                                {userPreference === 'male' && <Check size={24} className="text-pink-500" />}
                            </button>

                            <button
                                onClick={() => handlePreferenceSelect('female')}
                                className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${userPreference === 'female' ? 'border-pink-500 bg-pink-50 shadow-lg shadow-pink-100' : 'border-gray-100 bg-gray-50/80 hover:border-gray-200'}`}
                            >
                                <div className={`p-3 rounded-full transition-all ${userPreference === 'female' ? 'bg-pink-500 text-white' : 'bg-white text-pink-500'}`}>
                                    <Venus size={24} />
                                </div>
                                <span className="text-lg font-bold text-gray-800 flex-1 text-left">Women</span>
                                {userPreference === 'female' && <Check size={24} className="text-pink-500" />}
                            </button>

                            <button
                                onClick={() => handlePreferenceSelect('both')}
                                className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${userPreference === 'both' ? 'border-pink-500 bg-pink-50 shadow-lg shadow-pink-100' : 'border-gray-100 bg-gray-50/80 hover:border-gray-200'}`}
                            >
                                <div className={`p-3 rounded-full transition-all ${userPreference === 'both' ? 'bg-pink-500 text-white' : 'bg-white text-pink-500'}`}>
                                    <Users size={24} />
                                </div>
                                <span className="text-lg font-bold text-gray-800 flex-1 text-left">Everyone</span>
                                {userPreference === 'both' && <Check size={24} className="text-pink-500" />}
                            </button>
                        </div>
                        ...
                        <button
                            onClick={() => setStep(1)}
                            className="mt-4 text-sm font-medium text-gray-400 hover:text-gray-600 transition flex items-center justify-center gap-1"
                        >
...
const FeatureHint = ({ icon: Icon, title, description, position, side = "left", color = "pink" }) => {
    const sideClass = side === 'left' ? 'right-full mr-6' : 'left-full ml-6';

    const colorClasses = {
        pink: { bg: 'bg-pink-50', border: 'border-pink-200', icon: 'bg-pink-500 text-white', dot: 'bg-pink-500' },
        blue: { bg: 'bg-blue-50', border: 'border-blue-200', icon: 'bg-blue-500 text-white', dot: 'bg-blue-500' },
        purple: { bg: 'bg-purple-50', border: 'border-purple-200', icon: 'bg-purple-500 text-white', dot: 'bg-purple-500' },
        amber: { bg: 'bg-amber-50', border: 'border-amber-200', icon: 'bg-amber-500 text-white', dot: 'bg-amber-500' },
    };

    const colors = colorClasses[color] || colorClasses.pink;

    return (
        <div className={`absolute ${position} ${sideClass} z-[300] w-[220px] animate-in fade-in slide-in-from-bottom-2 duration-500 pointer-events-none`}>
            <div className={`${colors.bg} border ${colors.border} rounded-2xl p-4 shadow-lg relative`}>
                {/* Connecting line */}
                <div className={`absolute top-1/2 -translate-y-1/2 ${side === 'left' ? '-right-6' : '-left-6'} flex items-center`}>
                    <div className={`w-4 h-0.5 ${colors.dot}`}></div>
                    <div className={`w-2 h-2 ${colors.dot} rounded-full animate-pulse`}></div>
                </div>

                <div className="flex items-start gap-3">
                    <div className={`${colors.icon} p-2 rounded-xl shrink-0`}>
                        <Icon size={18} />
                    </div>
                    <div>
...
export default function DatingApp() {
  ...
  // View Change & Feature Hints Logic
  useEffect(() => {
      // Reset all hints when view changes
      setShowHomeTour(false);
      setShowChatTour(false);
      setShowMapTour(false);
      setShowProfileTour(false);

      if (currentView === 'home' && userName) {
          if (viewMode === 'stack') {
            setShowHomeTour(true);
            const timer = setTimeout(() => setShowHomeTour(false), 6000);
            return () => clearTimeout(timer);
          } else if (viewMode === 'map') {
            setShowMapTour(true);
            const timer = setTimeout(() => setShowMapTour(false), 6000);
            return () => clearTimeout(timer);
          }
      } else if (currentView === 'chat') {
          setShowChatTour(true);
          const timer = setTimeout(() => setShowChatTour(false), 6000);
          return () => clearTimeout(timer);
      } else if (currentView === 'profile') {
          setShowProfileTour(true);
          const timer = setTimeout(() => setShowProfileTour(false), 6000);
          return () => clearTimeout(timer);
      }
  ...
  const handleSwipe = (direction) => {
    ...
    setTimeout(() => {
      ...
      // Always increment to move to next card (or trigger end of stack)
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-200 font-sans text-gray-900 sm:py-20 overflow-x-hidden">
      ...
      <div className="relative">
              ...
              {viewMode === 'map' ? (
                ...
                <div className="flex-1 relative w-full h-full z-0 overflow-hidden">
                    ...
                    {filteredProfiles.length > 0 && currentIndex >= filteredProfiles.length && (
                        <div className="absolute inset-0 z-[60] bg-gradient-to-b from-pink-100 via-white to-rose-100 flex items-center justify-center p-6 animate-in fade-in duration-500">
                            {/* Decorative background elements */}
                            <div className="absolute top-20 left-10 w-32 h-32 bg-pink-200 rounded-full blur-[80px] opacity-60"></div>
                            <div className="absolute bottom-32 right-10 w-40 h-40 bg-rose-200 rounded-full blur-[100px] opacity-50"></div>
                            <div className="absolute top-1/3 right-1/4 w-24 h-24 bg-purple-200 rounded-full blur-[60px] opacity-40"></div>

                            <div className="bg-white/90 backdrop-blur-xl rounded-[40px] p-8 shadow-2xl max-w-sm w-full text-center border border-white/50 animate-in zoom-in-95 duration-500">
                                {/* Animated heart icon */}
                                <div className="relative w-24 h-24 mx-auto mb-6">
                                    <div className="absolute inset-0 bg-gradient-to-br from-pink-500 to-rose-500 rounded-full opacity-20 animate-ping"></div>
                                    <div className="relative w-24 h-24 bg-gradient-to-br from-pink-500 to-rose-500 rounded-full flex items-center justify-center shadow-lg shadow-pink-200">
                                        <Heart size={44} className="text-white" fill="white" />
                                    </div>
                                </div>

                                {/* Text content */}
                                <h2 className="text-3xl font-black text-gray-900 mb-2">Wow!</h2>
                                <p className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-pink-600 to-rose-500 mb-4">
                                    You're out of matches
                                </p>
                                <p className="text-gray-500 mb-8 leading-relaxed">
                                    You've seen everyone for now.<br/>
                                    <span className="font-semibold text-gray-700">Come back tomorrow</span> to discover new people!
                                </p>
                                ...
                                <div className="space-y-3">
                                    <button
                                        ...
                                        className="w-full py-4 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-2xl font-bold text-lg shadow-lg shadow-pink-200 hover:scale-[1.02] hover:shadow-xl transition-all duration-300"
                                    >
                                        Start Over
                                    </button>
                                    <button
                                        onClick={() => setCurrentView('matches')}
                                        className="w-full py-4 bg-gray-100 text-gray-700 rounded-2xl font-bold text-sm hover:bg-gray-200 transition-all"
                                    >
                                        View Your Matches
                                    </button>
                                </div>
                                ...
                                <div className="flex justify-center gap-1 mt-6">
                                    <Heart size={12} className="text-pink-300" fill="#f9a8d4" />
        ...
        {/* --- FEATURE HINTS (External - appear outside phone frame) --- */}
        {/* Home Tour - Stack View */}
        {showHomeTour && (
            <>
                <FeatureHint
                    icon={MapIcon}
                    title="Discover Nearby"
                    description="Switch to map view to explore matches in your area"
                ...
                <FeatureHint
                    icon={Info}
                    title="Learn More"
                    description="Tap to see their full profile, interests, and lifestyle"
        ...
        {showMapTour && (
            <>
                <FeatureHint
                    icon={Navigation}
                    title="Map Explorer"
                    description="Find people nearby! Tap any pin to view their profile"
        ...
        {showChatTour && (
            <>
                <FeatureHint
                    icon={Palette}
                    title="Style Your Chat"
                    description="Personalise with custom themes, fonts, and colors"
                    position="top-20 left-0"
                    side="right"
                    color="purple"
                />
                <FeatureHint
                    icon={CalendarHeart}
                    title="Plan a Date"
                    description="Use the date planner to suggest a perfect meetup"
        ...
        {/* Profile Tour - Premium */}
        {showProfileTour && (
            <>
                <FeatureHint
                    icon={Sparkles}
                    title="Lovr Premium"
                    description="See who likes you, unlimited rewinds & more exclusive features"
